...
- Slice 005 is done. `<Providers />` is in `layout.tsx`. `starwarsApi` and `teamApi` exist. Generated types and Zod are present under `src/gen/`.
- The smoke `useGetAllCharactersQuery()` consumer from Slice 005's `page.tsx` exists and is about to be replaced.
...
2. **Wire master resolution into `TeamService`** at `apps/platform/src/server/services/teamService.ts`. Replace the `new Map()` placeholder from Slice 004. New helper next to it:
   ...
   ```
   Calls `fetchCharacter(id)` for each id in `masters`, dedupes, returns the map. A single round of fetches is enough; we do not recurse on the masters' own masters. If any of those fetches return `null`, that master is omitted from the map (rule 3 just can't fire for it).
3. **Add a request-scoped character cache** at `apps/platform/src/server/akabab.ts`. Replace the bare `fetchCharacter` with `createCharacterFetcher()` that returns a `(id) => Promise<Character | null>` closure with an internal `Map<number, Promise<Character | null>>` so a single request never fetches the same id twice. The service factory in `createTeamService()` calls this once per request and threads the fetcher through.
4. **Build `<CharacterList />`** at `apps/platform/src/components/CharacterList.tsx` (single-consumer, stays in `apps/platform`). Reads `useGetAllCharactersQuery()`, renders `<StatePanel variant="loading|error">` for the in-flight and failed states, and a responsive grid of `<CharacterCard />` (from Slice 003) for the success state. Each card links to `/characters/[id]` using `next/link`.
...
6. **Build the detail page** at `apps/platform/src/app/characters/[id]/page.tsx`. Server component reads the route param, passes it to a client `<CharacterDetail id={id} />`. The client component:
   ...
   - Calls `useGetAllCharactersQuery()` for the list (already cached after a visit to `/`), uses it for two things: prev/next navigation and the `masterNames` map needed by `isDarkSide`.
...
- [ ] `TeamService.add()` resolves masters via `akabab` and threads them into `isDarkSide`
