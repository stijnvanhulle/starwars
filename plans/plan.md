...
Transform the current `TypeScript` library monorepo into a `Next.js` app that browses Star Wars characters from `akabab` and lets the user assemble a max-5 team while excluding evil characters. Characters are fetched directly from `akabab` via `RTK Query`. Only the team persists, in `Postgres` via `Drizzle`, served by our own `Next.js` route handlers behind a `Repository` → `Service` → handler layering. The schema has two tables: `teams` (seeded with a single `default` row) and `team_members` (FK → `teams.id`); the app uses the default team everywhere, so multi-team is a future feature, not a migration.
