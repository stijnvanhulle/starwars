...
5. **Build the detail page** at `apps/platform/src/app/characters/[id]/page.tsx`. Server component reads the route param, passes it to a client `<CharacterDetail id={id} />`. The client component:
   ...
   - Renders an `<ActionButton />` (from Slice 003) labelled "Add to team" / "Remove from team" based on `useGetTeamQuery()` membership. If `isDarkSide(character)` is true and the character is not already on the team, the button is disabled and `disabledReason="Evil characters cannot join the team."` (the exact tooltip text comes from `design.md`). The banner + button live inline in `CharacterDetail` (a red-tinted box wrapping the `<ActionButton />`); no separate `<ActionBar />` component is exported — see `design.md` component inventory. Clicks dispatch `addTeamMember` / `removeTeamMember`; on `422`/`409` errors, show the server's message inline.
