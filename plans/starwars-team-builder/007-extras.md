...
- Hits a bookmark icon on a character detail page; it flips filled; the navigation rail shows a count badge.
- Refreshes the browser; the bookmarked characters survive (re-hydrated from `localStorage`).
...
12. **Page-size sanity check on the team page** (optional polish). The team page can stay un-paginated; cap is 5 so the list never grows past 5 rows.
...
2. Refresh the browser; the bookmark survives. `localStorage.getItem('whale.bookmarks.v1')` matches.
...
7. `pnpm typecheck`, `pnpm lint`, `pnpm test` are green. Slice covers the five reducer cases; `paginate` helper covers the empty / first / middle / overshoot windows; `/bookmarks` smoke test passes.
