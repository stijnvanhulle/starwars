...
2. **Delete `packages/core` and `packages/demo`.** Remove both directories. In root `tsconfig.json`, replace the two stale `paths` aliases (`@whale/template-core` and `@whale/template-demo`) with `@whale/components → ./packages/components/src/index.ts` so IDE go-to-definition and inline typechecking resolve to the source. Remove any `cspell.json` allow-list entries that only existed because of those packages. Update `.gitignore` to cover `apps/*/.next/`, `apps/*/.turbo/`, and `apps/*/node_modules/` if not already matched by an existing wildcard.
3. **Scaffold `apps/platform`** with `package.json` (`name: "@whale/platform"`, `private: true`, `type: "module"`), `tsconfig.json` extending `configs/base.json`, and Next.js's standard layout. Use the App Router. Pin `next@16.2.6`, and let `react` / `react-dom` resolve to whatever Next.js 16.2.6 ships with. Add the `dev`, `build`, `start`, `lint`, `typecheck` scripts. Notes:
...
9. **Update root scripts.** Each root script becomes a plain `turbo run <task>` (no `--filter`), and turbo picks up whichever workspaces define the task. Adjust the root `package.json`:
   - `dev`: `turbo run dev` (only `apps/platform` defines `dev` at this point)
   - `build`: `turbo run build` (drops the current `--filter` list)
   - `start`: `turbo run start` (only `apps/platform` defines `start`, and Slice 007's Playwright `webServer` runs `pnpm start`)
   - `typecheck`: `turbo run typecheck --continue`
   - `test`: `turbo run test` (replaces the current root-level vitest invocation, and each workspace runs its own `test` script, so `apps/platform` runs Vitest via the config defined in step 10)
   ...
   - `test:e2e`: `{ "dependsOn": ["build"], "cache": false, "outputs": ["playwright-report/**", "test-results/**"] }`. Slice 007's CI workflow runs `pnpm test:e2e` from the root after `pnpm build`, and `dependsOn: ["build"]` keeps that contract.
...
13. **Update the release workflow** at `.github/workflows/release.yml`. The pre-existing workflow uses `changesets/action@v1`. Make two changes for this slice: extend the `paths` filter to include `apps/**` (so changesets that touch `apps/platform` still fire the workflow), and update the `get-version` job's `script` to look up `@whale/components` instead of the deleted `@whale/template-core`. The product is private (apps don't publish), but the Changesets entry still drives the `CHANGELOG.md` and version bump for `apps/platform` and `packages/components`.
...
1. `pnpm install` completes without errors and resolves `@whale/components` as a workspace link in `apps/platform/node_modules`.
...
- [x] `apps/platform/vitest.config.ts` and `apps/platform/playwright.config.ts` both parse cleanly, and `pnpm test` and `pnpm test:e2e -- --list` succeed
