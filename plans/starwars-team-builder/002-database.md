...
2. **Install `drizzle-orm` `^0.45.2`, `drizzle-kit` `^0.31.10`, `pg` `^8.21.0`, and `@types/pg` `^8.20.0`** in `apps/platform`. (Versions live in `plan.md`'s Technical Context table, and install is left to the slice author per the project rule.)
...
7. **Generate the initial migration** by running `drizzle-kit generate` (the slice author runs this once, and the generated `src/db/migrations/0000_*.sql` file is checked in). Do not hand-write the SQL.
...
11. **Write the repository tests** next to the repositories under `apps/platform/src/server/repositories/`. Two files. Each test resolves the default team via `teamRepository.findDefault()` at the top and reuses its `id`.
    ...
    - `findByTeamAndCharacterId(defaultId, 42)` returns the row, and `findByTeamAndCharacterId(defaultId, 999)` returns `undefined`.
