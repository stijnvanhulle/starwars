...
`Kubb` generates one frontend client + `Zod` from `plan/starwars-team-builder/contracts/api.openapi.yaml`. That contract documents every endpoint the browser calls (characters proxy routes and team routes), so the frontend has a fully generated client and never imports `starwars.openapi.yaml`. A second pipeline against `plan/starwars-team-builder/contracts/starwars.openapi.yaml` emits server-only types + `Zod` for the source-API shape used by the proxy fetcher.

## Phases

- **Planning Phases**: produce md files in `plan/starwars-team-builder/` (`spec.md`, `research.md`, `data-model.md`, `contracts/*.yaml`, `verification.md`, `design.md`, and the slice files).
...
- **Independent phases**: Each `plan/starwars-team-builder/00X-*.md` boots from a fresh `pnpm install` and ends in a runnable, demoable state.
...
**Output**: `plan/starwars-team-builder/spec.md`, `plan/starwars-team-builder/research.md`.
...
**Output**: `plan/starwars-team-builder/data-model.md`, `plan/starwars-team-builder/contracts/api.openapi.yaml`, `plan/starwars-team-builder/contracts/starwars.openapi.yaml`, `plan/starwars-team-builder/verification.md`.

The contract specs are checked into `plan/starwars-team-builder/contracts/` and copied to `apps/platform/openapi/` when Slice 004 (mirrors `api.yaml`) and Slice 005 (mirrors `starwars.yaml`) run.

## Planning Phase 2: Task split into Execution Slices

The slice files in `plan/starwars-team-builder/00X-*.md` are this feature's `tasks.md` equivalent. They share a skeleton defined in `plan/starwars-team-builder/template.md`.

### Execution Slices (one file each)

| Slice file                 | Depends on                                          | Demoable outcome                                                                                                                                                                                                                             |
| -------------------------- | --------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `plan/starwars-team-builder/001-setup.md`       | none                                                | `pnpm dev` serves an `MUI`-themed `Next.js` page at `localhost:3000`. `pnpm typecheck`/`lint`/`test` green. `packages/core` and `packages/demo` removed.                                                                                     |
| `plan/starwars-team-builder/002-database.md`    | 001                                                 | `docker compose up -d postgres && pnpm --filter platform db:migrate` creates `teams` and `team_members` and seeds the default team. Repository CRUD covered by `Vitest` integration tests against a real `Postgres`.                         |
| `plan/starwars-team-builder/003-design.md`      | 001                                                 | Design tokens land in the `MUI` theme (palette, typography, spacing, radius). `packages/components` exports the layout shell (`<AppShell />`, `<TeamSidebar />` placeholder, `<CharacterCard />`, `<StatePanel />` for loading/empty/error). |
| `plan/starwars-team-builder/004-api.md`         | 002 + Planning Phase 1 `api.yaml` + `starwars.yaml` | `/api/characters` and `/api/team` route handlers. Proxy uses a server-only `starwars-api` fetcher; team handlers go through `Service` + `Repository`                                                                                         |
| `plan/starwars-team-builder/005-client-kubb.md` | 003 + 004                                           | `pnpm --filter platform gen` produces `src/gen/api/` (types + client + `Zod`) and `src/gen/starwars/` (types + `Zod`, server-only). `store.ts` wires the single `api` RTK Query slice into `<Providers>`.                                    |
| `plan/starwars-team-builder/006-features.md`    | 005                                                 | All UI requirements live: list, detail with prev/next, persistent `<TeamSidebar />`, `/team`, real `isDarkSide`, evil-Add disabled. Screens built from the 003 components. `Vitest` covers `isDarkSide` and key components.                  |
| `plan/starwars-team-builder/007-testing.md`     | 006                                                 | `Playwright` specs cover browse/team/cap/evil. CI runs `Postgres` service container, migrations, unit + integration + e2e. One `Changesets` entry.                                                                                           |

## Planning Phase 3: Frontend Design

**Output**: `plan/starwars-team-builder/design.md` plus optional artifacts under `plan/starwars-team-builder/design/`.
...
1. **ASCII (required).** Sketch the three screens and the layout shell directly in `plan/starwars-team-builder/design.md`.
2. **Claude design (optional).** Generate mockups via Claude's frontend-design / Artifacts when visual questions remain. Drop into `plan/starwars-team-builder/design/`.
3. **Wireframes (optional, formal).** Produce responsive wireframes with state coverage. Drop into `plan/starwars-team-builder/design/`.

Gate for Slice 003: `plan/starwars-team-builder/design.md` has tokens, layout shell, one sketch per screen, component inventory, state matrix.
...
- [x] `plan/starwars-team-builder/spec.md`: done, user scenarios + FR-1..FR-7 + acceptance checklist
- [x] `plan/starwars-team-builder/research.md`: done, 6-row decisions table moved out of `plan.md`

### Planning Phase 1: Design & Contracts

- [x] `plan/starwars-team-builder/data-model.md`: done, `TeamMember` columns + `Character` fields + invariants + `isDarkSide` rules
- [x] `plan/starwars-team-builder/contracts/api.openapi.yaml`: done, the single frontend-facing spec covering `/api/characters`, `/api/characters/{id}`, `/api/team*`, and the shared `Character` schema
- [x] `plan/starwars-team-builder/contracts/starwars.openapi.yaml`: done, `starwars-api` spec, server-only (used by the proxy fetcher's types)
- [x] `plan/starwars-team-builder/verification.md`: done, 6 user-flow scenarios mapped to AC-1..AC-9

### Planning Phase 2: Slice files authored

- [x] `plan/starwars-team-builder/001-setup.md`: done
- [x] `plan/starwars-team-builder/002-database.md`: done
- [x] `plan/starwars-team-builder/003-design.md`: done
- [x] `plan/starwars-team-builder/004-api.md`: done
- [x] `plan/starwars-team-builder/005-client-kubb.md`: done
- [x] `plan/starwars-team-builder/006-features.md`: done
- [x] `plan/starwars-team-builder/007-testing.md`: done

### Planning Phase 3: Frontend Design

- [x] `plan/starwars-team-builder/design.md`: done. ASCII pass: tokens + layout shell + three screen sketches + component inventory + state matrix
- [x] `plan/starwars-team-builder/design/` hi-fi mockups: done. `home.html`, `character-detail.html`, `team.html` generated via Claude design from the tokens in `design.md`
...
- [ ] `plan/starwars-team-builder/research.md` open items closed or moved to follow-up issues
