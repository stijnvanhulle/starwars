...
| `plans/starwars-team-builder/002-database.md`    | 001                                                 | `docker compose up -d postgres && pnpm --filter platform db:migrate` creates `teams` and `team_members` and seeds the default team. Repository CRUD covered by `Vitest` integration tests against a real `Postgres`.                         |
...
| `plans/starwars-team-builder/005-client-kubb.md` | 003 + 004                                           | `pnpm --filter platform gen` produces `src/gen/api/` (types + client + `Zod`) and `src/gen/starwars/` (types + `Zod`, server-only). `store.ts` wires the single `api` RTK Query slice into `<Providers>`.                                    |
