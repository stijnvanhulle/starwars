...
import { ERRORS, TEAM_CAP } from '@/server/constants'
...
export async function addTeamMember({ memberRepo, fetchCharacter, isDarkSide, teamId, characterId }: AddTeamMemberParams): Promise<TeamMember> {
  ...
  if (character === null) {
    throw createError({ ...ERRORS.NOT_FOUND, message: `Character ${characterId} does not exist.`, data: { characterId } })
  }
  if (isDarkSide(character)) {
    throw createError({
      ...ERRORS.EVIL_FORBIDDEN,
  ...
  if (current >= TEAM_CAP) {
    throw createError({ ...ERRORS.TEAM_FULL, message: `The team already has ${TEAM_CAP} members.`, data: { cap: TEAM_CAP } })
  }
